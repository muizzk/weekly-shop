...
describe('Category e2e test', () => {
  ...
  it('should create and save Categories', async () => {
    async function createCategory() {
      ...
      await categoryUpdatePage.setOwnerInput('owner');
      expect(await categoryUpdatePage.getOwnerInput()).to.match(/owner/);
      await categoryUpdatePage.setOrderInput('5');
      expect(await categoryUpdatePage.getOrderInput()).to.eq('5');
  ...
  it('should delete last Category', async () => {
    ...
    expect(await categoryDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/weeklyShopApp.category.delete.question/);
